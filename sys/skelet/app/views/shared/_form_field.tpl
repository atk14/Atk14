...
{else}	
	...
	<div{if $field->required || $field->errors() || $class} class="{trim}{if $field->required}required{/if}{if $field->errors()} error_field{/if}{if $class} {$class}{/if}{/trim}"{/if}>
		...
		<label for="{$field->id_for_label()}"{if $hide_label} class="access"{/if}>{$field->label}{if $field->required}<span> ({t}povinné{/t})</span>{/if}</label>
		{if !$reverse}{$field->as_widget()}{/if}
		...
		{if $field->help_text || $field->hint}
			<div class="help">
				...
				{if $field->hint}
					<p class="hint"><strong>{t}Pøíklad:{/t}</strong> {$field->hint}</p>
				{/if}
			</div>
		{/if}
