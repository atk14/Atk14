...
animals: none
		...
		$this->assertEquals(3,count($ar["fruits"]));
		...
		$this->assertEquals(2,count($ar["vegetables"]));
...
";
		...
		$this->assertEquals(0,count($ar["empty_array"]));
...
key2: <?php echo "value 2"?>';
		...
		$ar = miniYAML::Load($data,["interpret_php" => true]);
		$this->assertEquals(["key1" => "value 1", "key2" => "value 2"],$ar);
$data = '
---
<?php for($i=1;$i<5;$i++){ ?>
...
';
		...
		$a = miniYAML::Load($data,["nullable" => false]);
		//
...
key2: "NULL"
		...
		$yaml = miniYAML::Dump($data,["nullable" => false]);
		//
		$this->assertEquals(trim('
---
key1: ""
