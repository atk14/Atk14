...
function __class_autoload__($options_or_class_name){
	...
	if(is_string($options_or_class_name)){
		...
		$_filenames = array(
			$class_name,
			preg_replace('/([a-z])([A-Z])/','\1_\2',$class_name), // RedFruit -> red_fruit; GoogleTagManagerB2C -> google_tag_manager_b2c
			preg_replace('/([a-z0-9])([A-Z])/','\1_\2',$class_name), // Ean13NumberField -> ean13_number_field
		);
